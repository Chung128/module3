...
INSERT INTO nhan_vien (
    ...
    so_dien_thoai, email, dia_chi, ma_vi_tri,
...
INSERT INTO dich_vu (
    ...
    tieu_chuan_phong, mo_ta_tien_nghi_khac, dien_tic_ho_boi, so_tang,
...
INSERT INTO hop_dong (ma_hop_dong, ngay_lam_hop_dong, ngay_ket_thuc_hop_dong, tien_dat_coc, ma_nhan_vien, ma_khach_hang, ma_dich_vu) VALUES
...
select * 
from khach_hang kh 
where timestampdiff(year,kh.ngay_sinh,curdate())>18 and timestampdiff(year,kh.ngay_sinh,curdate())<50 and (kh.dia_chi like '%Đà Nẵng' or kh.dia_chi like '%Quảng Trị');

-- 4 Đếm xem tương ứng với mỗi khách hàng đã từng đặt phòng bao nhiêu lần. Kết quả hiển thị được sắp xếp tăng dần theo số lần đặt phòng của khách hàng. Chỉ đếm những khách hàng nào có Tên loại khách hàng là “Diamond”.
select kh.ma_khach_hang,kh.ho_ten,count(hd.ma_hop_dong) as so_lan_thue 
from khach_hang kh 
...
select kh.ma_khach_hang,kh.ho_ten,lk.ten_loai_khach,hd.ma_hop_dong,dv.ten_dich_vu,hd.ngay_lam_hop_dong,hd.ngay_ket_thuc_hop_dong,ifnull((dv.chi_phi_thue+ifnull(sum(hdct.so_luong*dvdk.gia),0)),0) as tong_tien
from khach_hang kh
...
and  dv.ma_dich_vu not in (select dich_vu.ma_dich_vu
							from dich_vu 
...
count(distinct hd.ma_khach_hang) as so_lan_thue,
sum(ifnull(dvdk.gia*hdct.so_luong,0)) as gia_dich_vu_di_kem,
sum(ifnull(dv.chi_phi_thue,0)) as gia_dich_vu_chinh,
sum(ifnull(dvdk.gia*hdct.so_luong,0)+ifnull(dv.chi_phi_thue,0)) as tong_doanh_thu
from hop_dong hd
...
select hd.ma_hop_dong,hd.ngay_lam_hop_dong,hd.ngay_ket_thuc_hop_dong,hd.tien_dat_coc,sum(ifnull(hdct.so_luong,0)) as so_luong
from hop_dong hd
left join hop_dong_chi_tiet hdct on hdct.ma_hop_dong=hd.ma_hop_dong
...
select dvdk.* ,lk.ten_loai_khach,kh.ho_ten
from dich_vu_di_kem dvdk
 join hop_dong_chi_tiet hdct on hdct.ma_dich_vu_di_kem=dvdk.ma_dich_vu_di_kem
join hop_dong hd on hd.ma_hop_dong=hdct.ma_hop_dong
join khach_hang kh on kh.ma_khach_hang=hd.ma_khach_hang
join loai_khach lk on lk.ma_loai_khach=kh.ma_loai_khach
where lk.ma_loai_khach=1 and kh.dia_chi like '%Vinh' or kh.dia_chi like '%Quảng Ngãi';
...
select hd.ma_hop_dong,nv.ho_ten as ten_nhan_vien,kh.ho_ten as ten_khach_hang,kh.so_dien_thoai as sdt_khach,dv.ten_dich_vu,sum(ifnull(hdct.so_luong,0))as so_luong,hd.tien_dat_coc as coc_thang_3
from hop_dong hd
join nhan_vien nv on nv.ma_nhan_vien=hd.ma_nhan_vien
join  khach_hang kh on hd.ma_khach_hang=kh.ma_khach_hang
join dich_vu dv on dv.ma_dich_vu =hd.ma_dich_vu
left join hop_dong_chi_tiet hdct on hdct.ma_hop_dong=hd.ma_hop_dong
where year(ngay_lam_hop_dong)= 2020 and
     month(ngay_lam_hop_dong)in (10,11,12) and
     dv.ma_dich_vu not in (select hd.tien_dat_coc
							from hop_dong hd
							join nhan_vien nv on nv.ma_nhan_vien=hd.ma_nhan_vien
							join  khach_hang kh on hd.ma_khach_hang=kh.ma_khach_hang
							join dich_vu dv on dv.ma_dich_vu =hd.ma_dich_vu
							left join hop_dong_chi_tiet hdct on hdct.ma_hop_dong=hd.ma_hop_dong
                            where year(ngay_lam_hop_dong)= 2021 and
								  month(ngay_lam_hop_dong) in (1,2,3,4,5,6))
group by hd.ma_hop_dong
order by hd.ma_hop_dong;

-- 13 Hiển thị thông tin các Dịch vụ đi kèm được sử dụng nhiều nhất bởi các Khách hàng đã đặt phòng. (Lưu ý là có thể có nhiều dịch vụ có số lần sử dụng nhiều như nhau).
select dvdk.*,sum(ifnull(hdct.so_luong,0)) as so_luong_dich_vu_di_kem
from dich_vu_di_kem dvdk 
join hop_dong_chi_tiet hdct on hdct.ma_dich_vu_di_kem=dvdk.ma_dich_vu_di_kem
group by dvdk.ma_dich_vu_di_kem
having sum(hdct.so_luong) in (select max(tong_so_luong)
							from (
